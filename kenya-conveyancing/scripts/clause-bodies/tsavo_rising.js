// ─────────────────────────────────────────────────────────────────────────────
// Tsavo Rising - Verbatim Document Body
// Source: DRAFT SALE AGREEMENT - TSAVO RISING LIMITED
// Reference: MAK source DOCX (`_mak_source_templates/DRAFT SALE AGREEMENT-
//            TSAVO RISING.docx`), plus the audited per-project drafting rules at
//            scripts/drafting-rules/developer_sale_agreement__tsavo_rising.md
//
// Distinctive features of this template (most Vendor-protective in portfolio):
//   - Freehold absolute proprietor (NAIROBI/BLOCK 225/279)
//   - Construction in progress + Change of User in progress
//   - APCD: 30 September 2025
//   - Sale Completion Date lag: 180 BUSINESS DAYS (2x portfolio standard)
//   - Late-payment grace: 7 BUSINESS DAYS (strictest in portfolio)
//   - Negotiation Period DISAPPLIED for non-payment breaches
//   - Service Charge: BI-ANNUALLY (not monthly), KSh 2,000/mo x 6 = KSh 12,000
//   - Tenant-in-occupation handover: 30 days notice to tenant before handover
//   - Defects Liability Period: 6 months
//   - Vendor executes via Power of Attorney
//   - Dispute resolution: NCIA mediation -> court
//   - COVID audio-visual verification clause included
//
// Rules:
//   - Verbatim from MAK source. Project-deterministic content from
//     developer-projects.json (vendor company, title number, bank details).
//   - Dynamic per-matter content from d.* (purchaser name, etc.).
...
module.exports = function buildBody({ project, d, helpers }) {
  ...
  // ── WHEREAS ───────────────────────────────────────────────────────────────
  out.push(bodyPara([antiqua('WHEREAS:', { bold: true })]));
  out.push(spacer());

  // (A) Land - freehold, absolute proprietor
  out.push(makRecital([
    antiqua(`The Vendor is registered as the absolute proprietor of `),
    antiqua('ALL THAT', { bold: true }),
    antiqua(` property known as ${land.title_number} (subject to the conditions contained in the said Certificate of Title (hereinafter collectively referred to as "the `),
    antiqua('Land', { bold: true }),
    antiqua('")'),
  ]));

  // (B) Construction + Change of User in progress
  out.push(makRecital([
    antiqua('The Vendor has commenced and shall undertake the construction of the Development (as defined below) and the Change of User (as defined below) on the Land.'),
  ]));

  // (C) Geo-reference and register Sectional Plan after APCD
  out.push(makRecital([
    antiqua('Subject to completion of the Change of User, the Vendor shall geo-reference the Development and prepare and register the Sectional Plan (as defined below) after the Anticipated Practical Completion Date (as defined below) in accordance with Condition 4.7 of this Agreement.'),
  ]));
  ...
  def('Anticipated Practical Completion Date', `${d.apcd_override || '30th September 2025'} being the anticipated date of issue by the Architect of a certificate of practical completion confirming completion of all works in respect to the Development and "Practical Completion Certificate" shall be construed accordingly;`);
  ...
  // Change of User - composite definition with 2 sub-bullets
  out.push(makSubC([
  ...
  def('Data Protection Laws', 'the Data Protection Act No. 24 of 2019, Laws of Kenya and the Data Protection Regulations (as defined below);');
  def('Data Protection Regulations', 'together the Data Protection (General) Regulations 2021, the Data Protection (Complaints Handling Procedure and Enforcement) Regulations 2021 and the Data Protection (Registration of Data Controllers and Processors) Regulations 2021;');
  def('Defects Liability Period', 'the period of six (6) months from and including the Anticipated Practical Completion Date (defined above) in which the Vendor is required to repair material defects that appear in the Apartment(s) subject to the provisions of Conditions 2.6 and 2.7 below;');
  def('Development', `the Vendor’s development to be known as "${dev.name}" which shall be constructed to comprise Twelve (12) floors consisting of Studios, Superior Studios, One Bedrooms, Two bedrooms residential apartments together with social amenities like shops, pathways, driveways, parking spaces, gardens, and other usual amenities in accordance with the Building Plans and Architectural Drawings which are available for inspection at the Vendor’s offices;`);
  def('Land Act', 'the Land Act (Act No.6 of 2012), Laws of Kenya;');
  ...
  def('Land Registration Act', 'the Land Registration Act (Act No. 3 of 2012), Laws of Kenya;');
  def('Registrar', 'the relevant Land Registrar at the Lands Office, Nairobi;');

  // Sale Completion Date - 180 BD lag with 6 sub-bullets (unique to Rising)
  out.push(makSubC([
    antiqua('Sale Completion Date', { bold: true }),
    antiqua(' means the date falling at least one hundred and eighty (180) Business Days following the Anticipated Practical Completion Date (as defined above) subject to:'),
  ], 2));
  ...
  out.push(bulletPara([antiqua('the Change of User being completed by the Vendor;')]));
  out.push(definitionPara([antiqua('operations and processing timeline of Sectional Titles at the land registry; and')]));
  ...
  def('Service Charge', 'a bi-annually sum that shall be payable after the Anticipated Practical Completion Date by the Purchaser to the Corporation subject to Condition 4.1 and the By-Laws, which Service Charge shall be utilized for purposes of managing the Development;');
  ...
  out.push(makSubC([antiqua('The Vendor shall undertake the construction of the Development substantially in accordance with the approved Building Plans and the Architectural Drawings.')]));
  ...
  out.push(definitionPara([antiqua('will not affect the external facade to a material extent of the Development or the Apartment; or')]));
  out.push(definitionPara([antiqua('are required for obtaining any necessary licenses or permissions or complying with any requirement made by any local or other authority; or')]));
  out.push(definitionPara([antiqua('which the Vendor believes will improve the quality of accommodation or general appearance of the Development and/or the Apartment; and')]));
  ...
  out.push(makSubC([antiqua('The Vendor will (as between the Vendor and the Purchaser) ensure that any material defects in the Apartment which shall appear within the Defects Liability Period shall be repaired at the Vendor’s cost provided always that:')]));
  ...
  out.push(bodyPara([antiqua('and provided further that the Vendor shall not be liable to repair any material defects that relate to works undertaken by the Purchaser in fitting out the Apartment on taking possession or any defects which shall comprise deterioration of decoration occasioned by ordinary wear and tear.')]));

  out.push(makSubC([antiqua('In the event that possession of the Apartment has been granted to the Purchaser in accordance with Condition 4.1, the Vendor shall have the right with servants, agents, workmen and independent contractors and their plant, machinery and materials to enter the Apartment(s) upon at least forty-eight (48) hours prior notice (save in cases of emergency when less or no notice may be appropriate) for carrying out such works as are referred to in Condition 2.6.')]));
  out.push(spacer());
  ...
  // 3.5 Bank account
  out.push(makSubC([antiqua('All payments to the Vendor shall be paid to the following bank account or any other account advised by the Vendor in writing:')]));
  const vb = pay.vendor_bank;
  out.push(definitionPara([antiqua(`A/C NAME:         ${vb.account_name}`, { bold: true })]));
  out.push(definitionPara([antiqua(`A/C NO:		${vb.account_number}`, { bold: true })]));
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 4. GRANT OF POSSESSION, SALE COMPLETION, REGISTRATION AND PROPERTY MANAGEMENT
  // ─────────────────────────────────────────────────────────────────────────
  out.push(makClauseH('GRANT OF POSSESSION, SALE COMPLETION, REGISTRATION AND PROPERTY MANAGEMENT'));
  out.push(spacer());

  // 4.1 Conditions for Possession
  out.push(makSubC([antiqua('Conditions for Possession', { bold: true, underline: { type: UnderlineType.SINGLE } })]));
  out.push(bodyPara([antiqua('Subject to payment of (i) the Purchase Price in full as provided for in Condition Error! Reference source not found. and (ii) Stage 1 costs and Stage 2 costs in full as provided in Condition 3.1.4, the Vendor shall grant possession of the Apartment to the Purchaser upon the issuance of the Practical Completion Certificate and, for avoidance of doubt, in case the Purchaser is paying the Balance in instalments extending beyond the Anticipated Practical Completion Date, the Purchaser shall NOT be entitled to possession of the Apartment or any rental income accruing from the Apartment until full payment of the Balance is received by the Vendor in accordance with this Agreement. Upon the handover of possession of the Apartment by the Vendor to the Purchaser (whether directly to the Purchaser or indirectly to the Purchaser’s tenant), the Purchaser shall with effect from the first calendar month of receiving possession of the Apartment be responsible and ensure to remit to the Vendor and/or the Corporation (as the case may be) the Service Charge for the Apartment as and when it shall become due, whether formally demanded or not.')]));

  // 4.1 cont. - Tenant-in-occupation handover protocol (Rising-specific)
  out.push(bodyPara([antiqua('Where the Purchaser completes payment of the balance of Purchase Price and the said Apartment is in occupation by a Tenant, the Purchaser shall cause the Vendor to give the said tenant at least 30 days’ notice from the date of receipt of final payments to vacate the Apartment before the Apartment is officially handed over to the Purchaser subject to clause 4.2.')]));

  // 4.2 Post-handover inspection window
  out.push(makSubC([antiqua('Where the Purchaser is granted possession of the Apartment by the Vendor after the Defect Liability Period, then the Purchaser shall be afforded a period of thirty (30) days from the time of handover of the Apartment to carry out an inspection of the Apartment with the Vendor so as to agree on any reasonable repairs that may be required (if any) as a result of occupation by any tenant to return the Apartment to a reasonable state of repair and condition, save for ordinary wear and tear.')]));

  // 4.3 Apartment deemed sold
  out.push(makSubC([antiqua('Subject to Condition 4.1 and Condition 11 (Changes in Law or Circumstances), the Purchaser hereby acknowledges, confirms, and agrees that the Apartment will be deemed sold upon the terms and conditions of this Agreement, the Land Laws and any legal requirements under Kenyan law as at the Sale Completion Date.')]));

  // 4.4 Private residence use
  ...
  // 4.5 Sale completed at Vendor's Advocates
  out.push(makSubC([antiqua('The sale of the Apartment shall be completed at the offices of the Vendor’s Advocates on the Sale Completion Date.')]));

  // 4.6 Transfer instrument
  out.push(makSubC([
    antiqua('On the Sale Completion Date subject to Conditions 4.7 and 4.8 below, the Vendor shall issue an instrument of transfer (the '),
  ...
  out.push(makSubC([antiqua('copy of the National Identity Card of the Purchaser;')], 2));
  out.push(makSubC([antiqua('copy of the tax PIN Certificate of the Purchaser; and')], 2));
  out.push(makSubC([antiqua('three (3) coloured passport photos of the Purchaser.')], 2));

  // 4.7 Vendor responsible for Sectional Plan
  ...
  out.push(makSubC([antiqua('The Corporation shall be responsible for the management and administration of the Development and the Common Property subject to the provisions of the By-Laws. The membership rights in the Corporation shall be determined by the Unit Factor and upon registration of the Corporation, the Unit Factor shall be endorsed on the Sectional Title as evidence of membership in the Corporation.')], 2));
  out.push(makSubC([antiqua('Subject to Condition 4.8.3, upon the initial registration of the Corporation, the Corporation shall adopt the By-Laws. Any amendments to the By-Laws of the Corporation (as the case may be) shall be made in accordance with the provisions of Sectional Properties Laws.')], 2));
  out.push(makSubC([antiqua('The Purchaser hereby acknowledges that he/she shall be bound by the provisions of the By-Laws.')], 2));
  out.push(makSubC([antiqua('Subject to Condition 4.7.3, the Purchaser hereby acknowledges that he/she shall bear a proportionate share of the costs for registering the Corporation and its By-Laws.')], 2));

  // 4.9 Registration of Transfer
  out.push(makSubC([antiqua('Subject to registration of the Sectional Plan, issuance of the Sectional Title in favour of the Vendor and payment of "Stage 3 costs" as specified Condition 3.1.4, which are due on the Sale Completion Date, the Vendor’s Advocates shall undertake the registration of the Transfer within a reasonable period of time taking into account any delays in registration that may occur at the Lands Office, Survey of Kenya or any other relevant national or county government office.')]));

  // 4.10 Completion Documents
  ...
  out.push(makSubC([antiqua('original Sectional Title for the Apartment duly registered in favour of the Purchaser;')], 2));
  out.push(makSubC([antiqua('stamped and registered Transfer of the Apartment in favour of the Purchaser;')], 2));
  out.push(makSubC([antiqua('KRA payment e-slip in respect to payment of stamp duty on the Transfer;')], 2));
  out.push(makSubC([antiqua('certified copy of the Valuation for Stamp Duty Form (approved by the Government Valuer) for the Apartment;')], 2));
  ...
  out.push(makSubC([antiqua('certified copy of the duly registered Sectional Plan in respect to the Apartment(s); and')], 2));
  ...
  // 4.11 Property Management
  out.push(makSubC([antiqua('Property Management', { bold: true, underline: { type: UnderlineType.SINGLE } })]));
  out.push(bodyPara([
    antiqua('Upon grant of possession of the Apartment as provided under Condition 4.1, the Purchaser shall have the option of engaging '),
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // 5. TERMINATION OF AGREEMENT - Negotiation Period DISAPPLIED for non-payment
  // ─────────────────────────────────────────────────────────────────────────
  out.push(makClauseH('TERMINATION OF AGREEMENT'));
  out.push(spacer());

  out.push(makSubC([
    antiqua('Upon the occurrence of a breach of this Agreement (which is not a breach of a payment obligation in respect to the Purchase Price by the Purchaser), an injured party shall, prior to exercising their rights against a defaulting party, first attempt to address and resolve the breach with the defaulting party through good-faith negotiations for a period of thirty (30) days (the '),
  ...
  // 5.2 Purchaser's Default Event
  out.push(makSubC([antiqua('Purchaser’s Default Event', { bold: true, underline: { type: UnderlineType.SINGLE } })]));

  out.push(makSubC([
    antiqua('Subject to Condition 5.1 where applicable, if the Purchaser fails to comply with their obligations under this Agreement including the condition relating to payment of the Purchase Price, the Vendor may give the Purchaser notice in writing to comply with their obligations (the '),
    antiqua('Vendor’s Default Notice', { bold: true }),
    antiqua(') and require the Purchaser to make good the default within seven (7) Business Days, time being of the essence. On the failure of the Purchaser to comply with the Vendor’s Default Notice after its expiry, the Vendor may, without prejudice to its other rights or remedies, terminate this Agreement immediately by issuing a notice in writing to the Purchaser (the '),
    antiqua('Vendor’s Termination Notice', { bold: true }),
    antiqua(').'),
  ], 2));

  out.push(makSubC([antiqua('Upon issuance of the Vendor’s Termination Notice, the Vendor shall, within ninety (90) days from the date of issuance of the Vendor’s Termination Notice, refund to the Purchaser the amount paid on account of the Purchase Price, subject to deduction of all pending legal costs and thereafter this Agreement shall stand terminated notwithstanding the Purchaser’s refusal and/or failure to acknowledge the Vendor’s Termination Notice by signing it or otherwise and neither party shall have any claims against the other whether in tort, contract or otherwise.')], 2));

  out.push(makSubC([antiqua('Prior to termination of this Agreement, the Purchaser may identify other persons to purchase the Apartment on terms and conditions similar to this Agreement and if such new purchaser(s) shall enter into an agreement with the Vendor for the purchase of the Apartment at a price higher than the Purchase Price, then it is hereby agreed between the parties that the profit earned on the sale shall be paid to the Purchaser within thirty (30) days of completion of the sale of the Apartment to the new purchaser(s).')], 2));
  ...
  // 5.3 Vendor's Default Event
  out.push(makSubC([antiqua('Vendor’s Default Event', { bold: true, underline: { type: UnderlineType.SINGLE } })]));

  out.push(makSubC([
    antiqua('Subject to Condition 5.1, if (as the case may be) the Development is not completed by the  Anticipated Practical Completion Date or such other date as may be notified in writing by the Vendor to the Purchaser or there is a breach of any of the warranties specified in Condition 8 (the '),
    antiqua('Warranties', { bold: true }),
    antiqua(' and "'),
    antiqua('Warranty', { bold: true }),
    antiqua('" shall be construed accordingly) prior to the Sale Completion Date, then the Vendor shall inform the Purchaser of the delay in completion or breach of the Warranty and provide a timeline for completion or remedying of the breach of Warranty not exceeding ninety (90) days.'),
  ], 2));

  out.push(makSubC([
    antiqua('If the Development is still not completed or the breach of Warranty is still not remedied by the expiry of the timeline specified for completion or remedy (as the case may be) and provided always that any delay in completing the Development or remedying the breach of Warranty by the Vendor shall not in any way be as a result of or on account of circumstances beyond the Vendor’s control including the circumstances set out in Condition 7 (Force Majeure) below, then the Purchaser shall be entitled to issue the Vendor with a completion notice in writing specifying the default and requiring the Vendor to comply with its obligations within thirty (30) days of the date of the notice (the '),
    antiqua('Purchaser’s Default Notice', { bold: true }),
    antiqua(').'),
  ], 2));

  out.push(makSubC([
    antiqua('Failure of the Vendor to comply with the Purchaser’s Default Notice shall constitute a fundamental breach of this Agreement and the Purchaser may, without prejudice to any rights or remedies of the Vendor, terminate this Agreement by thirty (30) days’ notice in writing to the Vendor (the '),
    antiqua('Purchaser’s Termination Notice', { bold: true }),
    antiqua(') and upon expiry of the said Purchaser’s Termination Notice the Vendor shall refund all monies paid on account of the Purchase Price (without interest) to the Purchaser within ninety (90) days of expiry of the Purchaser’s Termination Notice and thereafter this Agreement shall stand terminated and neither party shall have any claims against the other whether in tort, contract or otherwise.'),
  ], 2));
  out.push(spacer());
  ...
  out.push(makSubC([antiqua('Notwithstanding the provisions of Condition 7.1, the obligations of the Purchaser to continue making payments to the Vendor in respect to the Purchase Price cannot be avoided or negated by the Purchaser claiming that any of the events or circumstances referred to in Condition 7.1 above have occurred unless evidence is provided by the Purchaser that the Force Majeure Event directly and adversely affects the Purchaser’s ability to pay.')]));

  out.push(makSubC([antiqua('Further to Condition 7.3, where a Force Majeure Event directly and adversely affects the Purchaser’s ability to pay thereby rendering it impossible for the Purchaser to fulfil their obligations under this Agreement during the subsistence of the Force Majeure Event, the Purchaser shall immediately notify the Vendor in writing and the parties shall engage in good faith negotiations to agree on an acceptable revised payment plan for the Purchase Price in accordance with Condition 5.1.')]));
  out.push(spacer());
  ...
  out.push(makSubC([antiqua('the Vendor is the legal and beneficial owner of the said Land;')], 2));
  ...
  out.push(makSubC([antiqua('The parties hereby agree that if any of the Warranties above is breached prior to the Anticipated Practical Completion Date then the provisions of Condition 5.3 shall apply.')]));
  out.push(spacer());
  ...
  out.push(spacer());
  out.push(bodyPara([antiqua('This Agreement is an executory agreement only and shall not operate or be deemed to operate as a lease of the Apartment.')]));
  out.push(spacer());

  // ─────────────────────────────────────────────────────────────────────────
  // 10. VERIFICATION OF EXECUTION VIA LIVE AUDIO-VISUAL LINK
  // ─────────────────────────────────────────────────────────────────────────
  ...
  out.push(spacer());
  out.push(makSubC([antiqua('The Purchaser confirms that the monies utilized to pay the Purchase Price for the Apartment and any other sums due from the Purchaser under this Agreement are not and shall not be from the proceeds of crime.')]));
  out.push(makSubC([antiqua('Failure by the Purchaser to adhere to the provisions of this Condition 13 shall be treated as a material breach of the provisions of this Agreement and if required by any Competent Authority shall entitle the Vendor, without prejudice to any other rights or remedies that the Vendor may have, to terminate this Agreement forthwith by issuance of written notice to that effect to the Purchaser and confiscate all monies paid by the Purchaser to the Vendor and submit such monies to the relevant Competent Authority or as required by law.')]));
  ...
  out.push(spacer());
  out.push(makSubC([antiqua('The Vendor hereby confirms to the Purchaser that all personal data given to the Vendor by the Purchaser and his/her Alternative Contact under this Agreement shall be used in accordance with the Data Protection Laws.')]));
  out.push(makSubC([antiqua('By signing this Agreement, the Purchaser hereby consents to his/her personal data being collected and processed for the purposes of execution, performance and completion of this Agreement.')]));
  out.push(spacer());
  ...
  out.push(spacer());
  out.push(bodyPara([antiqua('This Agreement may be executed in several counterparts, each of which will be deemed to be an original, but all of which, taken together, will constitute one and the same Agreement.')]));
  out.push(spacer());
  ...
  out.push(makSubC([antiqua('In the case of individuals, if the Purchaser passes away or becomes incapacitated during the term of this Agreement, then the Purchaser’s lawful representatives and/or heirs shall be the only parties recognized by the Vendor as capable of executing and/or benefitting from this Agreement and/or the Sectional Title. Where the Purchaser comprises more than one individual leaving behind other individual survivor(s) who are contracting parties, then the survivor(s) shall to all intents and purposes be recognized by the Vendor as the surviving contractual party under this Agreement and/or Sectional Title, subject to any legal process initiated by a deceased purchaser’s legal representatives under the Law of Succession Act (Cap.160).')]));

  out.push(makSubC([antiqua('The Purchaser acknowledges and admits that he/she entered into this Agreement on the basis of the Purchaser’s independent due diligence and not in reliance upon any representation made by or on behalf of the Vendor.')]));
  ...
  out.push(makSubC([antiqua('Notwithstanding the grant of the Sectional Title, this Agreement shall remain in force with regard to any obligations or restrictions hereunder not provided for in the Sectional Title.')]));
  ...
  out.push(makSubC([antiqua('Each of the parties hereby agrees and confirms, for the purposes of the Law of Contract Act (Chapter 23, Laws of Kenya), the Land Act, 2012 and the Land Registration Act, 2012 that it or he/she has executed this Agreement with the intention to bind itself or himself/herself to the contents hereof.')]));
