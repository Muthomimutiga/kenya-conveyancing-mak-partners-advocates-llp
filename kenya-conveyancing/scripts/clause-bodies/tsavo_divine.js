// ─────────────────────────────────────────────────────────────────────────────
// Tsavo Divine — Verbatim Document Body
// Source: DRAFT AGREEMENT FOR SALE — TSAVO DIVINE EXECUTIVE APARTMENTS LIMITED
// Reference: MAK source DOCX, word/document.xml (run-by-run bold/underline
//            verified against /tmp/tsavo_divine_runs.txt).
//
// Distinctive features of this template:
//   - Construction IN PROCESS — present-continuous recital ("is in the process
//     of erecting and completing")
//   - Change of User APPLIED, title surrender pending (Land is leasehold from
//     the Government of Kenya, Title Number Nairobi/Block 245/461)
//   - Anticipated Practical Completion Date: 28th February 2026
//   - Sale Completion Date: 90 Business Days AFTER APCD (5-bullet conditions
//     including Change of User)
//   - Defects Liability Period: SIX (6) months
//   - 10-day late-payment grace (calendar days)
//   - Highest Stage 2 costs in portfolio: water KES 9,000 / electricity KES
//     10,000 / service charge deposit KES 15,000
//   - Vendor executes via Power of Attorney
//   - Dispute resolution: NCIA mediation → Kenyan court (two-tier)
//   - Vendor bank: NCBA Bank Kenya PLC, Ciata Mall
//   - Vendor's Advocates bank: Equity Bank Kenya, Community Branch
//
// Verbatim rule (per memory/feedback_no_paraphrased_fallbacks.md):
//   Every clause's text is verbatim from the MAK source. Project-deterministic
//   content (vendor co. name, registration number, title number, development
//   name, bank account fields) is pulled from developer-projects.json so a
//   single registry edit propagates. Dynamic matter content comes from d.*.
//   No paraphrasing. No em dashes anywhere.
// ─────────────────────────────────────────────────────────────────────────────
...
module.exports = function buildBody({ project, d, helpers }) {
  ...
  // ── BETWEEN ───────────────────────────────────────────────────────────────
  out.push(bodyPara([
    antiqua('BETWEEN', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(':', { bold: true }),
  ]));
  out.push(spacer());
  ...
  out.push(makParty([
    ...
    antiqua(', being the person(s), whose particulars are set out in Section I of the Schedule of Particulars (which shall, where context allows, include the Purchaser’s successors in title, personal representatives, heirs and permitted assigns (as may be applicable) and where the Purchaser is more than one person, the Purchaser’s obligation shall be joint and several).'),
  ]));
  out.push(spacer());

  // ── WHEREAS ───────────────────────────────────────────────────────────────
  out.push(bodyPara([
    antiqua('WHEREAS', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(':', { bold: true }),
  ]));
  out.push(spacer());

  // Recital A — leasehold from Government of Kenya
  out.push(makRecital([
    antiqua('The Vendor is registered as proprietor as lessee from the Government of Kenya of '),
    antiqua('ALL THAT', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(` property known as Title Number ${land.title_number} (subject to the conditions contained in the said Certificate of Lease (hereinafter collectively referred to as "the Land").`, { underline: { type: UnderlineType.SINGLE } }),
  ]));

  // Recital B — Development in process of erection (present-continuous)
  out.push(makRecital([
    antiqua('The Vendor is in the process of erecting and completing a development project on the Land known as “'),
    antiqua('TSAVO DIVINE APARTMENTS', { bold: true }),
    antiqua('” (hereinafter referred to as “the '),
    antiqua('Development', { bold: true }),
    antiqua('”) which comprises 15 floors consisting of studio, Superior Studio and one bedroom residential apartments together with social amenities, pathways, driveways, commercial center, parking spaces, gardens, and other usual amenities in accordance with the Building Plans which are available for inspection at the Vendor’s offices.'),
  ]));

  // Recital C — Change of User applied, title surrender pending
  out.push(makRecital([
  ...
  // Helper: definitional sub-clause at level 2
  const def = (term, body) => {
    out.push(makSubC([
  ...
  def('Anticipated Practical Completion Date', `${d.apcd_override || '28th February 2026'} being the anticipated date of issue by the Architect of a certificate of partial or sectoral practical completion confirming completion of all works in respect to the Development and “Practical Completion Certificate” shall be construed accordingly;`);
  ...
  // Change of User — bullet sub-conditions
  out.push(makSubC([
  ...
  // Sale Completion Date — 90 BD from APCD, 5 sub-conditions
  out.push(makSubC([
    antiqua('Sale Completion Date', { bold: true }),
    antiqua(' means the date falling at least ninety (90) Business Days following the Anticipated Practical Completion Date (as defined above) '),
    antiqua('subject to:', { underline: { type: UnderlineType.SINGLE } }),
  ], 2));
  ...
  out.push(makSubC([
    antiqua('the “'),
    antiqua('Vendor', { bold: true }),
    antiqua('” shall include any person to whom the Vendor’s interest in the Land (or any part thereof) is transferred or assigned;'),
  ], 2));
  out.push(makSubC([
    antiqua('a “'),
    antiqua('party', { bold: true }),
    antiqua('” or to “'),
    antiqua('parties', { bold: true }),
    antiqua('” shall mean a party or the parties to this Agreement.'),
  ], 2));
  ...
  out.push(definitionPara([antiqua('will not affect the external facade to a material extent of the Development or the Apartment(s); or')]));
  ...
  out.push(makSubC([
    antiqua('The Vendor will (as between the Vendor and the Purchaser) ensure that any material defects in the Apartment(s) which shall appear within the Defects Liability Period shall be repaired at the Vendor’s cost '),
    antiqua('provided always that', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(':', { underline: { type: UnderlineType.SINGLE } }),
  ]));
  ...
  out.push(bodyPara([
    antiqua('and '),
    antiqua('provided further that', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(' the Vendor shall '),
    antiqua('not be liable to repair any material defects that relate to works undertaken by the Purchaser in fitting out the Apartment(s) on taking possession or any defects which shall comprise deterioration of decoration occasioned by ordinary wear and tear.', { bold: true, underline: { type: UnderlineType.SINGLE } }),
  ]));
  ...
  out.push(makSubC([
    ...
    antiqua('” due and payable at the time of execution of this Agreement;'),
  ], 2));
  out.push(makSubC([
    antiqua('“'),
    antiqua('Stage 2 costs', { bold: true }),
    antiqua('” due and payable within thirty (30) days of completion of payment of the Purchase Price. The Purchaser hereby acknowledges and accepts that the Stage 2 costs indicated in this Agreement are estimated amounts and the Vendor reserves the right to adjust these costs at the time scheduled for payment on the basis of the prevailing rate of inflation and actual expenses being incurred by the Vendor and/or the Corporation after the Anticipated Practical Completion Date and as such the final Stage 2 costs due from the Purchaser will be determined and communicated at the time the Purchaser is due to make payment and;'),
  ], 2));
  out.push(makSubC([
    antiqua('“'),
    antiqua('Stage 3 costs', { bold: true }),
    antiqua('” due and payable on the Sale Completion Date. It is hereby agreed that:'),
  ], 2));
  ...
  out.push(makSubC([
    antiqua('the Vendor shall be entitled to possession of the Apartment and to collect all rental income generated from the Apartment after the Anticipated Practical Completion Date until full payment of the Balance of the Purchase Price is received by the Vendor and such rental income shall '),
    antiqua('not be apportioned by the Purchasers towards settlement of the Purchase Price for any reason whatsoever.', { bold: true, underline: { type: UnderlineType.SINGLE } }),
  ], 2));

  // 3.5 — Vendor bank account (verbatim from registry)
  out.push(makSubC([antiqua('All payments to the Vendor shall be paid to the Vendors account bank account below;')]));
  const vb = pay.vendor_bank;
  out.push(definitionPara([
    antiqua('A/C NAME: ', { bold: true }),
    antiqua(vb.account_name, { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('A/C NO:  ', { bold: true }),
    antiqua(String(vb.account_number), { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('BANK: ', { bold: true }),
    antiqua(vb.bank_name, { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('BRANCH: ', { bold: true }),
    antiqua(String(vb.branch).toUpperCase(), { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('BANK CODE: ', { bold: true }),
    antiqua(String(vb.bank_code), { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('BRANCH CODE: ', { bold: true }),
    antiqua(String(vb.branch_code), { bold: true }),
  ]));
  out.push(definitionPara([
    antiqua('SWIFT CODE: ', { bold: true }),
    antiqua(vb.swift_code, { bold: true }),
  ]));
  out.push(spacer());

  // ─────────────────────────────────────────────────────────────────────────
  // 4. OVERDUE PAYMENTS (10-day grace)
  // ─────────────────────────────────────────────────────────────────────────
  ...
  out.push(makSubC([antiqua('Conditions for Possession', { underline: { type: UnderlineType.SINGLE } })]));
  out.push(bodyPara([
    antiqua('Subject to payment of (i) the Purchase Price in full as provided for in Condition 3.1 and (ii) Stage 1 costs and Stage 2 costs in full as provided in Condition 3.1.4, the Vendor shall grant possession of the Apartment to the Purchaser upon the issuance of the Practical Completion Certificate and, for avoidance of doubt, in case the Purchaser is paying the Balance in installments extending beyond the Anticipated Practical Completion Date, the Purchaser shall '),
    antiqua('not be entitled to possession of the Apartment or any rental income accruing from the Apartment until full payment of the Balance is received by the Vendor in accordance with this Agreement.', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(' Upon the handover of possession of the Apartment by the Vendor to the Purchaser (whether directly to the Purchaser or indirectly to the Purchaser’s tenant), the Purchaser shall with effect from the first calendar month of receiving possession of the Apartment be responsible and ensure to remit to the Vendor and/or the Corporation (as the case may be) the monthly Service Charge for the Apartment as and when it shall become due, whether formally demanded or not.'),
  ]));

  // 5.2 Post-Defects-Liability handover inspection
  out.push(makSubC([
    antiqua('Where the Purchaser is granted possession of the Apartment by the Vendor '),
    antiqua('after the Defect Liability Period, then the Purchaser shall be afforded a period of thirty (30) days from the time of handover of the Apartment(s) to carry out an inspection of the Apartment(s) with the Vendor so as to agree on any reasonable repairs that may be required (if any) as a result of occupation by any tenant to return the Apartment(s) to a reasonable state of repair and condition, save for ordinary wear and tear.', { bold: true, underline: { type: UnderlineType.SINGLE } }),
  ]));
  ...
  // 5.7 Vendor's Sectional Plan responsibility
  ...
  // 5.8 The Corporation (sub-heading)
  out.push(makSubC([antiqua('The Corporation', { underline: { type: UnderlineType.SINGLE } })]));
  ...
  out.push(makSubC([
    antiqua('Subject to registration of the Sectional Plan, issuance of the Sectional Title(s) in favour of the Vendor and payment of “'),
    antiqua('Stage 3 costs', { bold: true }),
    antiqua('” (as specified Condition 3.1.4) which are due on the Sale Completion Date, the Vendor’s Advocates shall undertake the registration of the Transfer within a reasonable period of time taking into account any delays in registration that may occur at the Lands Office, Survey of Kenya or any other relevant national or county government office.'),
  ]));

  // 5.10 Completion Documents (sub-heading)
  out.push(makSubC([antiqua('Completion Documents', { underline: { type: UnderlineType.SINGLE } })]));
  ...
  out.push(makSubC([antiqua('Property Management', { underline: { type: UnderlineType.SINGLE } })]));
  out.push(bodyPara([
  ...
  out.push(makSubC([antiqua('the Vendor shall, within ninety (90) days of the expiry of the Vendor’s Termination Notice, refund to the Purchaser the amount paid on account of the Purchase Price subject to deduction of all pending legal and thereafter this Agreement shall stand terminated and neither party shall have any claims against the other whether in tort, contract or otherwise;')], 2));
  ...
  // 6.3 — Development delay / breach of Warranty
  out.push(makSubC([
    antiqua('Subject to Condition 6.1, if (as the case may be) the Development is not completed by the  Anticipated Practical Completion Date or such other date as may be notified in writing by the Vendor to the Purchaser or there is a breach of any of the warranties specified in Condition  9 (the '),
    antiqua('Warranties', { bold: true }),
    antiqua(' and “'),
    antiqua('Warranty', { bold: true }),
    antiqua('” shall be construed accordingly) prior to the Sale Completion Date, then the Vendor shall inform the Purchaser of the delay in completion or breach of the Warranty and provide a timeline for completion or remedying of the breach of Warranty not exceeding ninety (90) days. If the Development is still not completed or the breach of Warranty is still not remedied by the expiry of the timeline specified for completion or remedy (as the case may be) and provided always that any delay in completing the Development or remedying the breach of Warranty by the Vendor shall not in any way be as a result of or on account of circumstances beyond the Vendor’s control including the circumstances set out in Condition 8 below, then the Purchaser shall be entitled to issue the Vendor with a completion notice in writing specifying the default and requiring the Vendor to comply with its obligations within thirty (30) days of the date of the notice (the '),
  ...
  out.push(definitionPara([antiqua('                                  NAIROBI', { underline: { type: UnderlineType.SINGLE } })]));
  out.push(definitionPara([antiqua(`			Email: ${v.notice_email}`, { underline: { type: UnderlineType.SINGLE } })]));
  out.push(spacer());
  out.push(bodyPara([
    antiqua('In the case of the Purchaser, to the address whose particulars are set out in Section I of the Schedule of Particulars.', { underline: { type: UnderlineType.SINGLE } }),
  ]));
  ...
  out.push(bodyPara([
    antiqua('PROVIDED ALWAYS THAT', { bold: true, underline: { type: UnderlineType.SINGLE } }),
    antiqua(' where a remedy exists under this Agreement to cater for the dispute concerned (be it an issue of interpretation, application or implementation), a party shall be obligated to adopt the agreed remedy available to the party under this Agreement.', { bold: true, underline: { type: UnderlineType.SINGLE } }),
  ]));
  out.push(spacer());
